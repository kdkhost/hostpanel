...
@section('title', 'Contratar ' . $product->name . ' — ' . config('app.name'))
        ...
        <div class="lg:col-span-2 space-y-5">
            ...
            <div class="kdk-card p-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    ...
                    <label class="relative cursor-pointer group">
                        <input type="radio" name="cycle" value="{{ $price->billing_cycle }}" x-model="cycle" class="sr-only peer">
                        <div class="border-2 rounded-xl p-4 text-center peer-checked:border-blue-600 peer-checked:bg-blue-50 group-hover:border-blue-300 transition">
                            <div class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">{{ $cycles[$price->billing_cycle] ?? $price->billing_cycle }}</div>
                            <div class="text-xl font-extrabold text-gray-900">R$ {{ number_format($price->price, 2, ',', '.') }}</div>
                            @php $m = $cycleMonths[$price->billing_cycle] ?? 1; @endphp
                            @if($m > 1)
                            <div class="text-xs text-gray-400 mt-0.5">R$ {{ number_format($price->price / $m, 2, ',', '.') }}/mês</div>
                            @endif
                        ...
                        <div class="absolute top-2 right-2 hidden peer-checked:flex">
                            <div class="w-5 h-5 bg-blue-600 rounded-full flex items-center justify-center"><i class="bi bi-check text-white text-xs"></i></div>
                        </div>
                    </label>
                    @endforeach
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
                    <label class="cursor-pointer">
                        ...
                        <div class="border-2 rounded-xl p-3 text-center peer-checked:border-blue-600 peer-checked:bg-blue-50 hover:border-blue-200 transition text-sm font-semibold">
                            <i class="bi bi-plus-circle block text-blue-500 mb-1 text-lg"></i>Registrar novo
                        </div>
                    </label>
                    <label class="cursor-pointer">
                        <input type="radio" name="domain_option" value="transfer" x-model="domainOption" class="sr-only peer">
                        <div class="border-2 rounded-xl p-3 text-center peer-checked:border-blue-600 peer-checked:bg-blue-50 hover:border-blue-200 transition text-sm font-semibold">
                            <i class="bi bi-arrow-left-right block text-green-500 mb-1 text-lg"></i>Transferir
                        </div>
                    </label>
                    <label class="cursor-pointer">
                        <input type="radio" name="domain_option" value="existing" x-model="domainOption" class="sr-only peer">
                        <div class="border-2 rounded-xl p-3 text-center peer-checked:border-blue-600 peer-checked:bg-blue-50 hover:border-blue-200 transition text-sm font-semibold">
                            <i class="bi bi-globe block text-purple-500 mb-1 text-lg"></i>Já possuo
                        </div>
                    </label>
                </div>
                <div x-show="domainOption === 'new'">
                    <div class="flex gap-2">
                        <input type="text" x-model="domainName" @input.debounce.500="checkDomain()"
                               class="flex-1 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-500" placeholder="meudominio">
                        <select x-model="domainTld" class="border border-gray-200 rounded-xl px-3 py-3 text-sm focus:outline-none focus:border-blue-500">
                            @foreach($tlds as $tld)<option value="{{ $tld->tld }}">.{{ $tld->tld }}</option>@endforeach
                        </select>
                    </div>
                    <div class="mt-2 text-sm" x-show="domainStatus">
                        <span x-show="domainStatus === 'available'" class="text-green-600 font-semibold"><i class="bi bi-check-circle-fill"></i> <span x-text="domainName + '.' + domainTld"></span> está disponível!</span>
                        <span x-show="domainStatus === 'taken'" class="text-red-500 font-semibold"><i class="bi bi-x-circle-fill"></i> Este domínio não está disponível.</span>
                        <span x-show="domainStatus === 'checking'" class="text-gray-400"><span class="inline-block w-3 h-3 border-2 border-gray-400 border-t-transparent rounded-full animate-spin"></span> Verificando...</span>
                    </div>
                </div>
                <div x-show="domainOption === 'transfer'">
                    <input type="text" x-model="domainName" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-500" placeholder="dominio.com.br">
                    <p class="text-xs text-gray-400 mt-1">Informe o domínio que deseja transferir para nós.</p>
                </div>
                <div x-show="domainOption === 'existing'">
                    <input type="text" x-model="domainName" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-500" placeholder="meudominio.com.br">
            ...
            <div class="kdk-card p-6">
                <h2 class="font-bold text-gray-900 mb-4 flex items-center gap-2">
                    ...
                    Cupom de Desconto <span class="text-xs text-gray-400 font-normal">(opcional)</span>
                </h2>
                <div class="flex gap-2">
                    ...
                    <button @click="validateCoupon()" :disabled="!couponCode || couponLoading"
        ...
        <div class="lg:col-span-1">
            <div class="kdk-card p-6 sticky top-20">
                ...
                <div class="border-t border-gray-100 pt-4 mb-5">
                    ...
                    <ul class="kdk-feature-list">
                        @foreach(array_slice(is_array($product->features) ? $product->features : json_decode($product->features ?? '[]', true), 0, 6) as $feat)
                        <li><i class="bi bi-check-circle-fill check"></i>{{ is_array($feat) ? ($feat['name'] ?? $feat['value'] ?? '') : $feat }}</li>
                        @endforeach
                    </ul>
                </div>
                @endif
                <button @click="addToCart()" :disabled="!cycle" class="btn-kdk-primary w-full justify-center">
                    <i class="bi bi-cart-plus"></i> Adicionar ao Carrinho
                </button>
                <p class="text-center text-xs text-gray-400 mt-3">
                    <i class="bi bi-shield-lock-fill text-green-500"></i> Pagamento 100% seguro
                </p>
            </div>
        </div>
    </div>
...
function orderProduct(productId, defaultCycle, prices) {
    return {
        ...
        async checkDomain() {
            ...
            try {
                const r = await fetch(`/api/dominio/verificar?domain=${encodeURIComponent(this.domainName + '.' + this.domainTld)}`, { headers: { 'Accept': 'application/json' } });
        ...
        addToCart() {
            ...
            cart.push({
                id: Date.now(),
                product_id: this.productId, product_name: '{{ addslashes($product->name) }}',
                product_slug: '{{ $product->slug }}', cycle: this.cycle, cycle_label: this.cycleLabel,
                price: this.cyclePrice, setup_fee: this.setupFee, total: this.total,
                domain: this.domainName ? (this.domainOption === 'new' ? this.domainName + '.' + this.domainTld : this.domainName) : '',
                domain_option: this.domainOption, coupon: this.couponValid ? this.couponCode : '', discount: this.discount,
            });
