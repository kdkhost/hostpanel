    ...
    <div class="max-w-5xl mx-auto px-4 py-3">
        <div class="flex items-center gap-0 text-xs font-semibold">
            <div class="flex items-center gap-1.5 text-green-600">
                <div class="w-6 h-6 rounded-full bg-green-500 text-white flex items-center justify-center text-xs"><i class="bi bi-check"></i></div>
                Configurado
            </div>
...
<div class="max-w-5xl mx-auto px-4 py-10" x-data="cartPage()">
    ...
    <div x-show="items.length === 0" x-cloak class="text-center py-24">
        <div class="w-20 h-20 bg-blue-50 rounded-2xl flex items-center justify-center mx-auto mb-5">
            <i class="bi bi-cart3 text-4xl text-blue-300"></i>
        </div>
        ...
        <a href="{{ route('store') }}" class="btn-kdk-primary">Ver Planos e Hospedagem →</a>
    </div>
    ...
    <div x-show="items.length > 0" x-cloak>
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1">
                <div class="kdk-card p-6 sticky top-20">
                    ...
                    <div class="space-y-2 text-sm mb-5">
                        ...
                        <div class="border-t border-gray-100 pt-3">
                            <div class="flex justify-between text-xs text-gray-400 mb-1" x-show="totalSetup > 0">
                                <span>Setup total</span><span x-text="'R$ ' + formatPrice(totalSetup)"></span>
                            </div>
                            <div class="flex justify-between text-xs text-green-600 mb-1" x-show="totalDiscount > 0">
                                <span>Descontos</span><span x-text="'- R$ ' + formatPrice(totalDiscount)"></span>
                            </div>
                    ...
                    <button @click="proceed()" class="btn-kdk-primary w-full justify-center mb-3">
                        Finalizar Pedido →
                    </button>
                    <p class="text-center text-xs text-gray-400">
                        <i class="bi bi-shield-lock-fill text-green-500"></i> Ambiente seguro e criptografado
                    </p>
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div class="p-6">
                <div class="flex items-center justify-between mb-6">
                    ...
                    <button @click="showCheckout=false" class="text-gray-400 hover:text-gray-600"><i class="bi bi-x-lg text-xl"></i></button>
                </div>
                <div class="flex border-b border-gray-200 mb-6">
                    <button @click="authTab='login'" :class="authTab==='login' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500'" class="flex-1 py-3 text-sm font-semibold transition">Já tenho conta</button>
                    <button @click="authTab='register'" :class="authTab==='register' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500'" class="flex-1 py-3 text-sm font-semibold transition">Criar conta</button>
                </div>
                <form @submit.prevent="doLogin()" x-show="authTab==='login'">
                    <div class="space-y-4">
                        <div><label class="block text-sm font-semibold text-gray-700 mb-1.5">E-mail</label>
                            <input type="email" x-model="loginForm.email" required class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-500" placeholder="seu@email.com"></div>
                        <div><label class="block text-sm font-semibold text-gray-700 mb-1.5">Senha</label>
                            <input type="password" x-model="loginForm.password" required class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-500"></div>
                        <p class="text-red-500 text-sm" x-show="loginError" x-text="loginError"></p>
                        <button type="submit" :disabled="authLoading" class="btn-kdk-primary w-full justify-center">
                ...
                <form @submit.prevent="doRegister()" x-show="authTab==='register'">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="sm:col-span-2"><label class="block text-sm font-semibold text-gray-700 mb-1.5">Nome Completo *</label>
                            <input type="text" x-model="regForm.name" required class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-500"></div>
                        <div class="sm:col-span-2"><label class="block text-sm font-semibold text-gray-700 mb-1.5">E-mail *</label>
                            <input type="email" x-model="regForm.email" required class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-500"></div>
                        <div><label class="block text-sm font-semibold text-gray-700 mb-1.5">Senha *</label>
                            <input type="password" x-model="regForm.password" required minlength="8" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-500"></div>
                        <div><label class="block text-sm font-semibold text-gray-700 mb-1.5">Confirmar Senha *</label>
                            <input type="password" x-model="regForm.password_confirmation" required class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-500"></div>
                        <div><label class="block text-sm font-semibold text-gray-700 mb-1.5">Telefone</label>
                            <input type="text" x-model="regForm.phone" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-500" placeholder="(11) 99999-9999"></div>
                        <div><label class="block text-sm font-semibold text-gray-700 mb-1.5">CPF / CNPJ</label>
                            <input type="text" x-model="regForm.document_number" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-500" placeholder="000.000.000-00"></div>
                    </div>
                    <p class="text-red-500 text-sm mt-3" x-show="regError" x-text="regError"></p>
                    <button type="submit" :disabled="authLoading" class="btn-kdk-primary w-full justify-center mt-4">
                        <span x-show="authLoading" class="inline-block w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin mr-1"></span>
...
function cartPage() {
    return {
        items: [], showCheckout: false, authTab: 'login', authLoading: false,
        ...
        init() { this.items = JSON.parse(localStorage.getItem('hostpanel_cart') || '[]'); }
    }
}
</script>
@endpush
